...
class PideInfoClient:
    ...
    async def sync_redsara_document(
        ...
        When ``access_request_id`` is provided (agent-driven submission flow
        in ``submit_request_reg``), it is included so the backend links the
        justificante to that pre-existing AR instead of creating a new one
        — the AR was generated by AI before REG returned the REGAGE, so
        ``expedienteRef`` lookup misses. The background Red SARA sync
        leaves this ``None``: it discovers registries with no known AR and
        relies on the backend's create-from-registry path.
        """
        ...
        if access_request_id:
            metadata["access_request_id"] = access_request_id
    ...
    async def sync_redsara_document_metadata_only(
        ...
        See ``sync_redsara_document`` for the ``access_request_id`` semantics.
        """
        metadata: dict = {
            "registryNumber": registro.registry_number,
            "registryStatus": registro.status,
